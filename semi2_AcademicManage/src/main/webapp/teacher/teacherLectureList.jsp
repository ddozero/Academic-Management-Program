...
<head>
  <meta charset="UTF-8">
  <title>강좌 세부 정보</title>
  <link rel="stylesheet" type="text/css" href="/semiTest/css/mainLayout.css">
  <style>
    .table-add { width: 100%; border-collapse: collapse; margin: 20px auto; border-radius : 10px; }
    .header-style { width: 15%; text-align: center; border: 1px solid #ddd; background-color: #eee; font-size : 15px; }
    .cell-data { width : 25%; text-align: left; padding: 10px; border: 1px solid #ddd; }
    .right-align { text-align: right; padding: 10px; }
    .button-add {
      padding: 6px 12px;
    ...
    .button-add:hover { background-color: #3457c0; }
  </style>
  <script>
    function enableEdit() {
...
<section class="all-section1">
  <form method="post" action="lectureIntroUpdate_ok.jsp">
    ...
    <table class="table-add">
      <tr><th class="header-style">강좌명</th><td class="cell-data" colspan="3"><%=dto.getClassname()%></td></tr>
      <tr>
      ...
      <tr>
        ...
        <td class="cell-data" colspan="3">
          ...
          <textarea id="classintroInput" name="classintro" rows="4" style="width:100%; resize:none; display:none;"><%=dto.getClassintro()%></textarea>
        </td>
      </tr>
      <tr>
        <td colspan="4" class="right-align">
          <button type="button" id="editBtn" class="button-add" onclick="enableEdit()">개요 수정</button>
          <input type="submit" id="saveBtn" class="button-add" style="display:none;" value="저장">
          <button type="button" id="cancelBtn" class="button-add" style="display:none;" onclick="cancelEdit()">취소</button>
        </td>
      </tr>
    </table>
  </form>
...
<div class="all-title1"><h2>세부 목차 목록</h2></div>
<section class="all-section1">
  <div class="right-align">
    <button class="button-add" onclick="window.open('weekInsertForm.jsp?classidx=<%=dto.getClassidx()%>', 'popup', 'width=600,height=500')">세부강좌 등록</button>
  </div>
  <table class="table-add">
    <tr>
    ...
    <tr>
      ...
      <td class="cell-data">
        <form method="post" action="weekDelete_ok.jsp" onsubmit="return confirm('정말 삭제하시겠습니까?');">
          <input type="hidden" name="classidx" value="<%=dto.getClassidx()%>">
          <input type="hidden" name="weeknum" value="<%=w.getWeeknum()%>">
          <input type="submit" value="삭제" class="button-add">
        </form>
      </td>
    </tr>
    <% } } else { %>
    <tr><td colspan="5" class="cell-data" style="text-align:center;">등록된 담당 강좌의 세부목차가 없습니다.</td></tr>
    <% } %>
  </table>
</section>
